...
def main():
    ...
    parser.add_argument('--use_amp',
                        action='store_true',
                        help="Use torch.cuda.amp (native mixed precision) on CUDA; no Apex required.")
    parser.add_argument('--max_train_steps',
                        type=int,
                        default=-1,
                        help="Stop after this many optimizer steps (-1 = full training). For smoke tests.")
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='Skip the per-test-triple filtered ranking loop (very slow on large KGs). '
             'Use external matching eval (e.g. real_comparison_kgbert_matching_eval.py) for main-protocol metrics.',
    )
    ...
    if args.use_amp and args.fp16:
        logger.warning("--use_amp and --fp16 both set; using native AMP only (Apex fp16 disabled).")
        args.fp16 = False

    use_amp_native = bool(args.use_amp) and args.do_train and device.type == 'cuda'
    if args.use_amp and device.type != 'cuda':
        logger.warning("use_amp requested but CUDA not used; training in fp32.")
        use_amp_native = False
    if use_amp_native and GradScaler is None:
        raise RuntimeError("torch.cuda.amp is unavailable; upgrade PyTorch or disable --use_amp.")

    logger.info("device: {} n_gpu: {}, distributed training: {}, fp16(Apex): {}, use_amp: {}".format(
        device, n_gpu, bool(args.local_rank != -1), args.fp16, use_amp_native))
    ...
    if args.do_train:
        ...
        model.train()
        scaler = GradScaler() if use_amp_native else None
        #print(model)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.skip_link_prediction:
            logger.info("***** Skipping built-in link prediction (--skip_link_prediction) *****")
            return
