...
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        # openai>=1.14 moved structured output to beta.chat.completions.parse;
        # chat.completions.parse may be absent (e.g. openai 1.91+).
        if "response_format" in kwargs:
            parse_fn = getattr(openai_async_client.chat.completions, "parse", None)
            if parse_fn is None:
                beta = getattr(openai_async_client, "beta", None)
                beta_cc = getattr(beta, "chat", None) if beta is not None else None
                if beta_cc is not None:
                    parse_fn = getattr(beta_cc.completions, "parse", None)
            if parse_fn is not None:
                response = await parse_fn(
                    model=api_model, messages=messages, **kwargs
                )
            else:
                # Compatible endpoints without .parse: JSON object mode + text body
                kw = dict(kwargs)
                kw.pop("response_format", None)
                kw.setdefault("response_format", {"type": "json_object"})
                response = await openai_async_client.chat.completions.create(
                    model=api_model, messages=messages, **kw
                )
        else:
            response = await openai_async_client.chat.completions.create(
                model=api_model, messages=messages, **kwargs
