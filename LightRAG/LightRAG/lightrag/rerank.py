...
async def generic_rerank_api(
    ...
    # DashScope (and similar) reject top_n > len(documents)
    _nd = len(documents) if documents else 0
    if top_n is not None and _nd > 0 and top_n > _nd:
        logger.debug(
            "Clamping rerank top_n from %s to %s (document count)",
            top_n,
            _nd,
        )
        top_n = _nd
    ...
    if request_format == "aliyun":
        ...
        # extra_body may reintroduce top_n > len(documents)
        _pn = payload["parameters"].get("top_n")
        if _pn is not None and _nd > 0 and _pn > _nd:
            payload["parameters"]["top_n"] = _nd
    else:
        ...
        _pn2 = payload.get("top_n")
        if _pn2 is not None and _nd > 0 and _pn2 > _nd:
            payload["top_n"] = _nd
