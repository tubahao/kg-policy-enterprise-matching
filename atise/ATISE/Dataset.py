...
class KnowledgeGraph:
    def __init__(self, data_dir, gran=1,rev_set=0):
        ...
        if self.data_dir == 'icews14':
            ...
            # 自定义数据目录：由 train/valid/test 中的日期自动设定起点与 n_time（避免沿用 ICEWS 导致 day 越界或尺度失真）
        ...
        self.load_dicts()
        if self.data_dir not in ('icews14', 'icews05-15'):
            self._configure_time_from_splits()
        self.load_triples()
        self.load_filters()
    ...
    def _configure_time_from_splits(self):
        """扫描 train/valid/test 第四列日期，设 start_sec 与 n_time=max_day+1。"""
        dates = []
        for fname in ('train.txt', 'valid.txt', 'test.txt'):
            ...
            for v in df.iloc[:, 3].tolist():
                dates.append(str(v).strip())
        secs = [time.mktime(time.strptime(d, '%Y-%m-%d')) for d in dates]
        self.start_sec = min(secs)
        self.start_date = time.strftime('%Y-%m-%d', time.localtime(self.start_sec))
        max_day = int((max(secs) - self.start_sec) / (self.gran * 24 * 60 * 60))
        self.n_time = max(max_day + 1, 1)
        print('-----Custom time axis-----')
        print('start_date: {}, n_time (day slots): {}'.format(self.start_date, self.n_time))
