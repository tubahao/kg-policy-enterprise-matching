...
def sample_negatives(X, C, kg):
    ...
    n_half = int(M * C / 2)
    X_corr[:n_half, 0] = np.random.randint(0, kg.n_entity, size=n_half)
    X_corr[n_half : M * C, 1] = np.random.randint(0, kg.n_entity, size=M * C - n_half)
...
def train(task ='LinkPrediction',
    ...
    if data_dir == 'yago' or data_dir == 'wikidata':
        ...
        # 自定义目录：entity2id.txt / relation2id.txt / train.txt / valid.txt / test.txt
        kg = KnowledgeGraph(data_dir=data_dir, gran=gran, rev_set=rev_set)
        n_day = kg.n_time
