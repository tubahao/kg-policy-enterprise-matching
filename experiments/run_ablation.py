...
# -*- coding: utf-8 -*-
"""
按 `E:\\Paper\\Kg\\消融实验.md` 设计执行 11 个消融实验，并与当前最佳结果对比。

输出：
- reports/ablation_results_11.csv
- reports/ablation_results_11.md
- reports/ablation_beta_sensitivity.csv

耗时与日志：
- 默认打印 [阶段 1/3][阶段 2/3][阶段 3/3] 与各 matcher 初始化耗时；每项消融后打印 [计时] E→P/P→E 秒数。
- 若仍觉「卡住」：多为连续初始化 2～3 个 matcher（各加载 BERT）。加 `--verbose` / `-v` 可显示评测内部进度。
- `--skip-d3` 跳过 β 扫参（少 6 次完整评估）。完整跑仍约 10～20+ 分钟属正常，不是死锁。
"""
...
| 多模态融合 | B1/B2/B3 | 去掉层级/时间衰减后 E→P 通常下降，B3 跌幅最大 | 衰减与多源融合均必要 |
| 图结构表征 | C1/C2 | C1 去掉 GAT 后按 BASE 语义:重要性比例重归一（约 0.818/0/0.182） | C2 为 GCN 嵌入替换 |
| 衰减机制 | D1/D2/D3 | 无衰减 / 线性衰减 / β 扫参作对照 | 以表内相对 BASE 的 Δ 为准 |
...
**修正**：去掉图结构时仅将 BASE 的 **语义:重要性 = 0.45:0.1** 归一化到和为 1（约 **0.818/0/0.182**）。见脚本中 C1 的 `params`。
...
| C1 | 清空 GAT；E→P **≈0.818/0/0.182**（重归一） | P→E 与 E→P 图信号均变 |
...
from matching.experiment_profiles import get_profile_eval_parameter_dict, load_frozen_average_metrics  # noqa: E402
from matching.policy_embedding_defaults import JOINT_EMB, JOINT_IDX, TITLE_EMB, TITLE_IDX  # noqa: E402
...
def _importance_parquet_path(importance_tag: str = "") -> Path:
    t = (importance_tag or "").strip()
    if t:
        return PROJECT_ROOT / "evaluation" / f"policy_importance_with_decay_{t}.parquet"
    return PROJECT_ROOT / "evaluation" / "policy_importance_with_decay.parquet"


def _load_base_importance_df(importance_tag: str = "") -> pd.DataFrame:
    p = _importance_parquet_path(importance_tag)
    if not p.is_file():
        raise FileNotFoundError(f"缺少重要性 parquet: {p}")
...
def _importance_map_from_mode(
    mode: str, beta_time: float = 0.05, beta_level: float = 0.2, importance_tag: str = ""
) -> Dict[int, float]:
    df = _load_base_importance_df(importance_tag)
    ...
    if mode == "default_combined":
        ...
        # 去掉层级衰减：仅用 parquet 的 time_decayed（与主表 --no_ppr_fusion 一致，末尾统一 _norm01，不读 PPR）。
        if "time_decayed" not in df.columns:
            score = base * np.exp(-beta_time * dt)
        else:
            score = df["time_decayed"].to_numpy(dtype=np.float64)
    elif mode == "no_time":
        if "level_decayed" not in df.columns:
            score = base * np.exp(-beta_level * dl)
        else:
            score = df["level_decayed"].to_numpy(dtype=np.float64)
    elif mode == "no_decay":
        # D1：不衰减即 V_chain = base（等价于 β_l=β_t=0 的指数链）。与当前主表 --no_ppr_fusion 一致，仅对 base 做后续 min-max，
        # 不读 graphrag/importance_scores.npy，避免「BASE 无 PPR、D1 却融合 PPR」的口径分裂。
        score = base.astype(np.float64)
    elif mode == "linear_decay":
        # 线性近似：仍不含 PPR（保留为粗粒度对照）；若需严格可比须扩展 policy_importance_with_decay
        lam_t = beta_time
        lam_l = beta_level
...
def _base_eval_params() -> Dict[str, Any]:
    # 与 `tune_main_matching_grid.py --round4` 子进程最优一致（见 matching/evaluation_results_main_tuned_round4.json）
    return {
        # E->P
        "top_k_policy": -1,
...
def _evaluate(
    ...
    *,
    ...
    """
    verbose_eval=True：不重定向 stdout，可看到 evaluate_matching 内进度条（终端会很长）。
    默认 False：静默评测，但在结束后打印 E→P / P→E 分段耗时，避免误以为「卡死」。
    """
    t_all = time.perf_counter()
    out_ctx = nullcontext() if verbose_eval else redirect_stdout(io.StringIO())
    with out_ctx:
    ...
    hint = "" if verbose_eval else "；加 --verbose 可打印评测内部进度"
    print(
        f"    [计时]{pre} E→P {t1 - t0:.1f}s | P→E {t2 - t1:.1f}s | 合计 {t2 - t_all:.1f}s{hint}",
        flush=True,
    )
    ep_avg = ep["average"]
    pe_avg = pe["average"]
...
def _build_matcher(
    ...
    company_emb_path: str,
    company_index_path: str,
    ...
    return BidirectionalMatcher(
        ...
        gat_artifact_tag=gat_artifact_tag,
    )
...
def _disable_gnn(matcher: BidirectionalMatcher) -> None:
    ...
    matcher.policy_query_matcher.gat_emb_by_idx = None
    matcher.policy_query_matcher.gat_emb_valid = None
...
def _replace_with_non_contrastive_graph_emb(matcher: BidirectionalMatcher) -> None:
    ...
    matcher.refresh_e2p_structure_from_gat_np(p)
...
def _snapshot_matcher_state(matcher: BidirectionalMatcher) -> Dict[str, Any]:
    ...
    snap = {
        ...
        "gat_emb_by_idx": pr.gat_emb_by_idx.clone() if pr.gat_emb_by_idx is not None else None,
        "gat_emb_valid": pr.gat_emb_valid.clone() if pr.gat_emb_valid is not None else None,
    }
    return snap
...
def _restore_matcher_state(matcher: BidirectionalMatcher, snap: Dict[str, Any]) -> None:
    ...
    ge = snap.get("gat_emb_by_idx")
    pr.gat_emb_by_idx = ge.clone() if ge is not None else None
    gv = snap.get("gat_emb_valid")
    pr.gat_emb_valid = gv.clone() if gv is not None else None
...
def run_quick_align_base_c1(seed: int = 42, *, cudnn_deterministic: bool = False) -> None:
    ...
    out_path = PROJECT_ROOT / "reports" / "ablation_base_c1_aligned.json"
...
def run_quick_c1() -> None:
    ...
    out_j = PROJECT_ROOT / "reports" / "ablation_c1_quick_metrics.json"
    out_j.parent.mkdir(parents=True, exist_ok=True)
    out_j.write_text(json.dumps(m, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"已写入: {out_j}")


def run(*, verbose_eval: bool = False, skip_d3: bool = False, skip_d_group: bool = False):
    t_run = time.perf_counter()
    _d3_line = "④ D3 β 敏感性再 ×6 次完整评估。"
    if skip_d_group:
        _d3_line = "④ 已 --skip-d，跳过 D1/D2/D3 整组（含 β 扫参）。"
    elif skip_d3:
        _d3_line = "④ 已 --skip-d3，跳过 β 扫参（仍跑 D1/D2）。"
    print(
        "\n[消融耗时说明] 主要时间花在：① 扫描三元组构建查询；② 每个 matcher 各加载一次 BERT+图；"
        "③ 每项消融 = 整表 E→P(330)+P→E(87)；"
        + _d3_line
        + "\n默认每项结束会打印 [计时]；长时间无输出时多半在「初始化 matcher」或「E→P BERT 编码」。"
        "需要看评测内部进度请加 --verbose。\n",
        flush=True,
    )

    print("[阶段 1/3] 构建测试查询（约 13 万条 supports 扫描）…", flush=True)
    t0 = time.perf_counter()
    enterprise_queries, policy_queries = build_test_queries_from_data(
        max_enterprise_queries=300, max_industry_queries=30, max_policy_queries=200
    )
    print(f"  -> 完成 {time.perf_counter() - t0:.1f}s | 企业侧查询 {len(enterprise_queries)} | 政策侧 {len(policy_queries)}", flush=True)

    df_pol = pd.read_parquet(PROJECT_ROOT / "data_intermediate" / "policies_clean.parquet")
    valid_policy_ids = set(df_pol["policy_id"].astype(int).tolist())

    base_params = get_profile_eval_parameter_dict("a2_base")
    ...
    print("[阶段 2/3] 检查 joint 政策嵌入（缺失时会现场跑 BERT，可能很慢）…", flush=True)
    t0 = time.perf_counter()
    ensure_joint_bert_policy_embeddings()
    ...
    title_paths = {
        "policy_emb_path": TITLE_EMB,
        "policy_index_path": TITLE_IDX,
        "company_emb_path": "embeddings/enterprise_text_emb.npy",
        "company_index_path": "embeddings/enterprise_index.json",
    }

    print("[阶段 3/3] 初始化 matcher（每个都会加载 BERT，通常各需数十秒～数分钟）…", flush=True)
    matchers: Dict[str, BidirectionalMatcher] = {}
    t0 = time.perf_counter()
    matchers["legacy"] = _build_matcher(**legacy_paths, gat_artifact_tag=None)
    print(f"  -> legacy（concat+默认GAT）{time.perf_counter() - t0:.1f}s", flush=True)
    t0 = time.perf_counter()
    matchers["a2"] = _build_matcher(**joint_paths, gat_artifact_tag="a2_joint")
    print(f"  -> a2（joint+a2_joint）{time.perf_counter() - t0:.1f}s", flush=True)
    matcher_snaps: Dict[str, Dict[str, Any]] = {k: _snapshot_matcher_state(v) for k, v in matchers.items()}

    a3_gat_p = PROJECT_ROOT / "graph" / "gat_policy_emb_contrastive_a3_title.npy"
    a3_gat_c = PROJECT_ROOT / "graph" / "gat_company_emb_contrastive_a3_title.npy"
    a3_imp = PROJECT_ROOT / "evaluation" / "policy_importance_with_decay_a3_title.parquet"
    if a3_gat_p.is_file() and a3_gat_c.is_file() and a3_imp.is_file():
        t0 = time.perf_counter()
        matchers["a3"] = _build_matcher(**title_paths, gat_artifact_tag="a3_title")
        matcher_snaps["a3"] = _snapshot_matcher_state(matchers["a3"])
        print(f"  -> a3_title {time.perf_counter() - t0:.1f}s", flush=True)
        has_a3 = True
    else:
        has_a3 = False
        print("警告: 缺少 A3 标题线全流程产物，跳过实验 A2（原 A3：仅标题向量+独立 GAT）", flush=True)

    frozen_base = load_frozen_average_metrics("a2_base")
    rows.append(
        {
            "id": "BASE",
            "name": "A2主实验BASE(冻结 evaluation_results_a2_joint_full_pipeline.json)",
            "category": "Baseline",
            **frozen_base,
        }
    )
    print(f"  BASE E->P NDCG={frozen_base['ep_ndcg']:.4f} F1={frozen_base['ep_f1']:.4f}")

    imp_a2 = "a2_joint"

    # C1：去掉结构分后，仅将 BASE 的「语义:重要性」重归一到和为 1（与 get_profile_eval_parameter_dict 当前口径一致）
    _c1_b_sem = float(base_params.get("policy_semantic_weight", 0.45))
    _c1_b_imp = float(base_params.get("policy_importance_weight", 0.1))
    _c1_den = _c1_b_sem + _c1_b_imp
    if _c1_den < 1e-9:
        _c1_den = 1.0
    _c1_sem_norm = _c1_b_sem / _c1_den
    _c1_imp_norm = _c1_b_imp / _c1_den

    experiments: List[Dict[str, Any]] = [
        {
            "id": "A1",
            "name": "独立编码后拼接(原主实验文本+默认GAT/衰减)",
            "category": "BERT嵌入策略",
            "matcher_key": "legacy",
            "params": {},
        },
    ...
    experiments.extend(
        [
            {
                "id": "B1",
                "name": "w/o Hierarchy(移除层级衰减)",
                "category": "多模态融合",
                "matcher_key": "a2",
                "params": {},
                "importance_mode": "no_hierarchy",
                "importance_tag": imp_a2,
            },
            {
                "id": "B2",
                "name": "w/o Time(移除时间衰减)",
                "category": "多模态融合",
                "matcher_key": "a2",
                "params": {},
                "importance_mode": "no_time",
                "importance_tag": imp_a2,
            },
            {
                "id": "B3",
                "name": "Shallow Fusion(语义单流线性)",
                "category": "多模态融合",
                "matcher_key": "a2",
                "params": {
                    "policy_semantic_weight": 1.0,
                    "policy_structure_weight": 0.0,
                    "policy_importance_weight": 0.0,
                    "policy_industry_boost": 0.0,
                },
            },
            {
                "id": "C1",
                "name": "w/o GNN(结构权重置0,语义/重要性按BASE比例重归一)",
                "category": "图结构表征",
                "matcher_key": "a2",
                "params": {
                    "policy_semantic_weight": _c1_sem_norm,
                    "policy_structure_weight": 0.0,
                    "policy_importance_weight": _c1_imp_norm,
                },
                "disable_gnn": True,
            },
            {
                "id": "C2",
                "name": "GCN替代GAT(无对比图嵌入)",
                "category": "图结构表征",
                "matcher_key": "a2",
                "params": {},
                "replace_graph_emb": True,
            },
            {
                "id": "D1",
                "name": "w/o Decay(β=0)",
                ...
                "matcher_key": "a2",
                "params": {},
                "importance_mode": "no_decay",
                "importance_tag": imp_a2,
            },
            {
                "id": "D2",
                "name": "Linear Decay替代Exponential",
                "category": "衰减机制",
                "matcher_key": "a2",
                "params": {},
                "importance_mode": "linear_decay",
                "importance_tag": imp_a2,
            },
        ]
    )
    ...
    if skip_d_group:
        experiments = [e for e in experiments if e.get("id") not in ("D1", "D2")]
        print("[跳过] D 组：已指定 --skip-d，不跑 D1、D2。", flush=True)

    print(f"开始执行 {len(experiments)} 项单次消融...")
    for i, exp in enumerate(experiments, start=1):
        print(f"[{i}/{len(experiments)}] {exp['id']} {exp['name']}")
        ...
        if exp.get("importance_mode"):
            imp = _importance_map_from_mode(
                exp["importance_mode"],
                importance_tag=exp.get("importance_tag", imp_a2),
            )
    ...
    if not skip_d3 and not skip_d_group:
        print("执行 D3 β 参数敏感性分析（6 次完整评估）…", flush=True)
        for beta in [0.01, 0.03, 0.05, 0.08, 0.10, 0.15]:
            matcher = matchers["a2"]
            _restore_matcher_state(matcher, matcher_snaps["a2"])
            imp = _importance_map_from_mode("beta_time_exp", beta_time=beta, beta_level=0.2, importance_tag=imp_a2)
            _apply_importance_map(matcher, imp)
            metrics = _evaluate(
                matcher,
                enterprise_queries,
        ...
        print(
            "[跳过] D3 β 扫参："
            + ("--skip-d（整组 D 已跳过）。" if skip_d_group else "已指定 --skip-d3。"),
            flush=True,
        )

    out_reports = PROJECT_ROOT / "reports"
    out_reports.mkdir(parents=True, exist_ok=True)
    out_beta = out_reports / "ablation_beta_sensitivity.csv"
    if beta_rows:
    ...
    id_order = ["BASE", "A1", "A2", "B1", "B2", "B3", "C1", "C2", "D1", "D2", "D3"]
    id_order = [x for x in id_order if x in set(df["id"].tolist())]
    ...
    out_csv = out_reports / "ablation_results_11.csv"
    ...
    out_md = out_reports / "ablation_results_11.md"
    base_note = (
        "基线：**A2 主实验 BASE**（指标冻结自 `matching/evaluation_results_a2_joint_full_pipeline.json`；"
        "B/C/D 在 **A2 图栈**（`a2_joint` GAT 与 `policy_importance_with_decay_a2_joint.parquet` 列重算）上消融）。\n\n"
    ...
    beta_md_line = (
        ...
        if beta_rows
            ...
            f"- β敏感性：本次未跑 D3（`--skip-d` 或 `--skip-d3`），`{out_beta}` 仅含表头。\n"
        )
    )
    out_md.write_text(
    ...
    print("\n全部完成。")
    print(f"- 本脚本总耗时: {total_s:.1f}s ({total_s/60:.1f} min)")
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="11 项消融或快速 C1 / 对齐 BASE 验收")
    ap.add_argument(
        "--quick-c1",
        action="store_true",
    ...
    ap.add_argument(
        ...
        "-v",
        action="store_true",
        help="评测时不重定向 stdout，可看到 E→P/P→E 内部进度（输出量很大）",
    )
    ap.add_argument(
        "--skip-d3",
        action="store_true",
        help="仅跳过 D3 β 扫参（D1/D2 仍跑；总表无 D3 行）",
    )
    ap.add_argument(
        "--skip-d",
        action="store_true",
        help="跳过衰减组 D1/D2 与 D3 β 扫参（总表无 D1/D2/D3 行）",
    )
    ns = ap.parse_args()
    if ns.quick_align_base_c1:
        ...
        run(
            verbose_eval=ns.verbose,
            skip_d3=ns.skip_d3,
            skip_d_group=ns.skip_d,
        )
