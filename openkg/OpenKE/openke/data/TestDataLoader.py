...
class TestDataLoader(object):
	...
	def read(self):
		...
		idx_dtype = np.int32 if platform.system().lower().startswith("win") else np.int64
		self.test_h = np.zeros(self.entTotal, dtype=idx_dtype)
		self.test_t = np.zeros(self.entTotal, dtype=idx_dtype)
		self.test_r = np.zeros(self.entTotal, dtype=idx_dtype)
		self.test_h_addr = self.test_h.__array_interface__["data"][0]
		self.test_t_addr = self.test_t.__array_interface__["data"][0]
		self.test_r_addr = self.test_r.__array_interface__["data"][0]

		self.test_pos_h = np.zeros(self.testTotal, dtype=idx_dtype)
		self.test_pos_t = np.zeros(self.testTotal, dtype=idx_dtype)
		self.test_pos_r = np.zeros(self.testTotal, dtype=idx_dtype)
		self.test_pos_h_addr = self.test_pos_h.__array_interface__["data"][0]
		self.test_pos_t_addr = self.test_pos_t.__array_interface__["data"][0]
		self.test_pos_r_addr = self.test_pos_r.__array_interface__["data"][0]
		self.test_neg_h = np.zeros(self.testTotal, dtype=idx_dtype)
		self.test_neg_t = np.zeros(self.testTotal, dtype=idx_dtype)
		self.test_neg_r = np.zeros(self.testTotal, dtype=idx_dtype)
