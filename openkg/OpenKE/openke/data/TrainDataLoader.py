...
class TrainDataLoader(object):
	...
	def read(self):
		...
		# Windows + mingw 构建的 Base.so 在写入索引时与 int64 内存布局不一致，
		# 会导致索引越界。这里改为 int32 以保证 ctypes 写入正确。
		idx_dtype = np.int32 if platform.system().lower().startswith("win") else np.int64
		self.batch_h = np.zeros(self.batch_seq_size, dtype=idx_dtype)
		self.batch_t = np.zeros(self.batch_seq_size, dtype=idx_dtype)
		self.batch_r = np.zeros(self.batch_seq_size, dtype=idx_dtype)
